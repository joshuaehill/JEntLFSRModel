...
This code also includes a program to model all the raw noise symbols in a range and then produce 1 million bytes of conditioned output per analyzed raw noise symbol by repeatedly LFSR processing the existing state. This is used to support the claim that the LFSR output is indisquishable from pseudorandom values using the SP 800-90B testing tools.
